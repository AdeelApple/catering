...
function filter(pg) {
  ...
  if ($("#scr").length < 1)
    $("body").append(
      "<div id='scr' class='screen'><i class='fas fa-3x fa-spinner fa-pulse'></i></div>"
    );
...
function create_user(btn) {
  ...
  if (fullname == "") {
    msg("fullname cannot be empty!", 2);
    return;
  }
  var username = $("#new_username").val();
  if (username == "") {
    msg("username cannot be empty!", 2);
    return;
  }
  var pass = $("#new_pass").val();
  if (pass == "") {
    msg("pass cannot be empty!", 2);
    return;
  }
  var type = $("#new_type").val();
  if (type == "") {
    msg("type cannot be empty!", 2);
    return;
  }
  ...
  $.post(
    ...
    function (d) {
      $(btn).html(oldtxt);
      if (d == "success") msg("User Created.");
      else msg("User Creation failed: " + d, 2);
    }
  );
}
...
function savePurchasedQty(obj) {
  ...
  if (old_content != new_content) {
    $.post(
      ...
      function (d) {
        if (d == "success") msg("Saved");
        else msg("Failed to Save changes." + d);
      }
    );
  }
}
...
function calRemainingQty(obj) {
  ...
  $(tr)
    .find(".remaining")
    .html(sub($(tr).find(".wtotal").html(), $(tr).find(".purchased").val()));
}
...
function save_settings() {
  ...
  $.post("ajax.php", sett, function (d) {
    if (d == "success") msg("All Changes Saved.");
    else msg("Failed to Save changes." + d);
  });
}
...
function update_users(btn) {
  ...
  if (update) {
    $.post("ajax.php", { arr: json, fun: 301 }, function (d) {
      $(btn).html(oldtxt);
      if (d == "success") msg("Changes saved.");
      else msg("Changes failed: " + d, 2);
    });
  } else {
    $(btn).html(oldtxt);
  }
...
function atblur(obj) {
  ...
  if (ncont != $(obj).attr("data-oldcont")) {
    ...
    $.post(
      "ajax.php",
      { id: tid, nm: ncont, clm: tclm, tbl: ttbl, fun: tfun },
      function (d) {
        if (d == "success") msg("Changes saved.");
        else msg("Changes failed: " + d, 2);
      }
...
function update_privileges(obj) {
  ...
  $.post(
    "ajax.php",
    { id: id, clm: clm, val: val, tbl: tbl, fun: 345 },
    function (d) {
      if (d == "success") msg("Changes saved.");
      else msg("Changes failed: " + d, 2);
    }
...
function del_user(btn) {
  ...
  $.post("ajax.php", { uid: id, fun: 400 }, function (d) {
    ...
    } else msg("User deletion failed: " + d, 2);
  });
}
// Delete Order
