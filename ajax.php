...
$funs = array(
	...
	4000 => function () {
		...
		<tr>
			...
			<td>
				<select name="item<?= $id . $iid; ?>" id="item<?= $n ?>" data-pkgid="<?= $id; ?>"
					onchange="<?= $cat['fun']; ?>pack_cat_change(this)" data-previous="" data-cls="<?= $id . $cat['id']; ?>"
					class="cat_select form-control form-control-sm" required="required">
			...
			<td class="pp-cell<?= $id . $cat['id'] ?> <?= $cls1 ?> ppinp">
				<input type="number" name="extra_qty<?= $id . $iid ?>" class="form-control form-control-sm extra_qty"
					placeholder="extra" step="any" min="0" oninput="cal_extra_qty($(this).closest('tr'));" <?= $read; ?>>
				<input type="hidden" name="extra_price<?= $id . $iid ?>" value="0.00" class="extra_price">
			</td>

			<!-- Trays Elements -->
			<td class="non-pp<?= $id . $cat['id'] ?> <?= $cls2; ?>"><input type="number" name="tray_lg<?= $id . $iid ?>"
					class="tray lg_tray" value="0" readonly="readonly"></td>
			<td class="non-pp<?= $id . $cat['id'] ?> <?= $cls2; ?>"><input type="number" name="tray_md<?= $id . $iid ?>"
					class="tray md_tray" value="0" readonly="readonly"></td>
			<td class="non-pp<?= $id . $cat['id'] ?> <?= $cls2; ?>"><input type="number" name="tray_sm<?= $id . $iid ?>"
					class="tray sm_tray" value="0" readonly="readonly"></td>
		...
		<tbody>
			<?php $n = 1;
				...
				<tr>
					...
					<?php if ($val['rowspan']) { ?>
						<td rowspan="<?= $val['rowspan'] ?>" class="align-middle font-weight-bold" contenteditable="true">
							<?= $val['description']; ?>
						</td>
						<td rowspan="<?= $val['rowspan'] ?>" class="align-middle font-weight-bold" contenteditable="true">
							<?= $val['total'] . $val['unit2']; ?>
		...
		<tbody>
			<?php
				...
				<tr>
					...
					<?php } ?>
					<td class="align-middle font-weight-bold" contenteditable="true"><span
							class="wtotal"><?= round($wval['wtotal'], 2) ?></span> <?= get_unit_for_report($wval); ?></td>
					<td class="align-middle font-weight-bold"><input type="number"
							class="form-control form-control-sm purchased" onchange="calRemainingQty(this)"
							onblur="savePurchasedQty(this)" data-old="<?= $wval['purchased']; ?>" data-id="<?= $wval['id']; ?>"
							value="<?= round($wval['purchased'], 1); ?>"></td>
					<td class="align-middle font-weight-bold"><span contenteditable="true"
							class="remaining"><?= round($wval['remaining'], 1) ?></span> <?= get_unit_for_report($wval); ?></td>
	...
	<tbody id="tbody">
		<?php $n = 1;
			while ($r = mysqli_fetch_array($rs)) {
				$oid = $r['id']; ?>
			<tr>
				<td><a href="booking_edit.php?oid=<?= $oid; ?>" class="btn-sm btn-success c-pointer"><i
							class="fa fa-pen"></i></a>
					<?php
						if ($r['is_pre_booking'] == 1) {
							?>
						<a href="pre_booking.php?oid=<?= $oid; ?>" class="btn-sm btn-primary c-pointer"><i
								class="fa fa-calendar-check"></i></a>
					<?php } else { ?>
						<a href="receipt.php?oid=<?= $oid; ?>" class="btn-sm btn-warning c-pointer"><i
								class="fa fa-file-invoice"></i></a>
					<?php } ?>
				</td>
				<td><?= $r['id'] ?></td>
				<td class="text-left"><span class="c-p h-blue font-weight-bold" data-toggle="collapse"
						data-target="#a<?= $n ?>"><?= $r['name'] ?></span></td>
				...
				<td valign="align-middle"><button onclick="del_order(this,<?= $oid; ?>)" class="btn btn-sm btn-danger"><i
							class="fa fa-xs fa-trash"></i></button></td>
			</tr>
							...
							<?php
								...
								<tr>
									...
									<td><?php if ($r1['type'] == 1)
											...
											echo "-"; ?></td>
									<?php if (!$r1['pp']) { ?>
										<td><?php if ($r1['tray_lg'] > 0)
												echo $r1['tray_lg']; ?></td>
										<td><?php if ($r1['tray_md'] > 0)
												echo $r1['tray_md']; ?></td>
										<td><?php if ($r1['tray_sm'] > 0)
												echo $r1['tray_sm']; ?></td>
									<?php } else { ?>
			...
			<div id="k-1" class="col border p-0 k-items">
				<?php foreach ($pots_list as $key => $val) { ?>
					<div class="black-border">
						<div class="text-center h4 p-2 font-weight-bold" contenteditable="true">
							<?= ($key + 1) . "- " . $val['potname'] . "(" . $val['potnum'] . "/" . $val['totalpots'] . ")<span class='mx-4'>" . date('h:iA', strtotime($val['delivery_time'])) . "</span>" ?>
						</div>
						<?php foreach ($val['potitems'] as $key1 => $v) { ?>
							<div class="row m-0 border">
								...
								<div class="col p-0" contenteditable="true">
									<?php if ($v['type'] == 1 || ($v['type'] == 3 && $v['meat_type'] == 1))
											echo intval($v['persons']) . 'pp'; ?>
								</div>
								<?php if ($v['pp']) { ?>
								...
								<?php } else {
										$trays = ""; ?>
									<?php if ($v['tray_lg'] > 0)
											$trays .= "<span class='px-2' contenteditable='true'>" . round($v['tray_lg'], 1) . 'L</span>'; ?>
									<?php if ($v['tray_md'] > 0)
											$trays .= "<span class='px-2' contenteditable='true'>" . round($v['tray_md'], 1) . 'M</span>'; ?>
									<?php if ($v['tray_sm'] > 0)
											$trays .= "<span class='px-2' contenteditable='true'>" . round($v['tray_sm'], 1) . 'S</span>'; ?>
						...
						<?php }if (isset($val['pot'])) { ?>
							<div class="row m-0 fc p-fc pot">
								<div class="col p-0 text-center" contenteditable="true">
									<?php if ($val['pot']['rice'] > 0)
											echo "Rice: " . round($val['pot']['rice'], 2) . "kg"; ?>
								</div>
								<div class="col p-0 text-center" contenteditable="true">
									<?php if ($val['pot']['meat'] > 0)
											echo "Meat: " . round($val['pot']['meat'], 2) . "kg"; ?>
								</div>
							</div>
						<?php } ?>
					</div>
						...
						<?php $ttl = 0;
							...
							<div class="row m-0 border">
								...
								<?php } else {
										$trays = ""; ?>
									<?php if ($v['tray_lg'] > 0)
											$trays .= "<span class='px-2'>" . round($v['tray_lg'], 1) . 'L</span>'; ?>
									<?php if ($v['tray_md'] > 0)
											$trays .= "<span class='px-2'>" . round($v['tray_md'], 1) . 'M</span>'; ?>
									<?php if ($v['tray_sm'] > 0)
											$trays .= "<span class='px-2'>" . round($v['tray_sm'], 1) . 'S</span>'; ?>
									<div class='col p-0'><?= $trays ?></div>
								<?php } ?>
	...
	6004 => function () {
		...
		<div class="col p-0">
			...
			<div id="k-1" class="col border p-0 k-items">
				<?php foreach ($pots_list as $key => $val) { ?>
					<div class="black-border">
						<div class="text-center h4 p-2 font-weight-bold">
							<?= $val['potname'] . "(" . $val['potnum'] . ")<span class='mx-4'>" . date('h:iA', strtotime($val['delivery_time'])) . "</span>" ?>
						</div>
						<?php foreach ($val['potitems'] as $key1 => $v) { ?>
							<div class="row m-0 border">
								...
								<div class="col p-0"><?php if ($v['type'] == 1)
										echo intval($v['persons']) . 'pp'; ?></div>
								<?php if ($v['pp']) { ?>
								...
								<?php } else {
									...
									<div class='col p-0'><?= $trays ?></div>
								<?php } ?>
								<div class="col p-0"><?php if ($v['type'] == 2)
										echo "(ctm)";
									echo $v['description']; ?></div>
							</div>
						<?php }if (isset($val['pot'])) { ?>
							<div class="row m-0 fc p-fc pot">
