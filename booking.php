...
<form action="do_files/do_booking.php" method="post" id="booking-form">
	<div class="container-fluid border">
		...
		<div class="row mt-2">
			<div class="col-md-4 py-1">
				<div class="input-group input-group-sm">
					...
					<input type="text" name="orderno" class="form-control" value="<?php echo order_no() ?>"
						disabled="disabled">
				</div>
			</div>
			<div class="col-md-4 py-1">
			...
			<div class="col-md-4 py-1">
				<div class="input-group input-group-sm">
					...
					<input type="number" name="persons" id="persons" value="25" min="15"
			...
			<div class="col-md-4 py-1">
				<div class="input-group input-group-sm">
					...
					<input type="datetime-local" id="delivery_time" name="delivery_time" class="form-control" value=""
						required="required">
					...
					<div class="form-row my-1">
						...
						<div class="col-md-3">
							<div class="input-group input-group-sm">
								...
								<input type="number" name="pkgprice<?= $id ?>" id="pkgprice<?= $id ?>" step="any"
									oninput="amount_cal()" class="form-control form-control-sm pkgprice" value="0.00">
							</div>
						</div>
						<div class="col-md-6">
							<div class="input-group input-group-sm">
								...
								<tr>
									...
									<?php if (!$ci['pp']) { ?>
										<td class="ctm_tray_price">
											<input type="number" id="tray_lg_ctm<?= $id . $iid ?>"
												...
												value="0" min="0" oninput="ctm_total(this)" readonly="readonly">
											<!-- price -->
									...
									<?php } else { ?>
										...
										<td colspan="2" class="ctm_tray_price">
											<input type="number" name="ctm_qty<?= $id . $iid ?>" id="ctm_qty<?= $id . $iid ?>"
												class="item item-qty form-control form-control-sm" min="0" value="0"
												oninput="ctm_pp_total(this)" readonly="readonly">
									...
									<td class="ctm_total_and_persons">
										<input type="number" name="total_price_ctm<?= $id . $iid ?>"
											id="ctm_total_price<?= $id . $iid ?>" class="tray ctm_total_price" step="any"
											data-active="0" value="0.00" readonly="readonly">
									</td>
									<td>
										<input type="number" name="d_total_price_ctm<?= $id . $iid ?>"
											class="d_ctm_total_price form-control form-control-sm" min="0" step="any"
											value="0.00" data-active="0" oninput="amount_cal()">
									</td>
									<td><input type="text" name="comment_ctm<?= $id . $iid ?>" id="comment_ctm<?= $id . $iid ?>"
											class="form-control form-control-sm">
										<input type="hidden" name="list<?= $id . $iid ?>" class="list_num" value="<?= $ci['list'] ?>">
									</td>
								</tr>
								<?php $n++;
							} ?>
			...
			<div id="full_custom<?= $i ?>" class="row fullctm collapse <?= $show ?>" data-show="<?= $dshow ?>">
				<div class="col-md-12">
					...
					<table id="t1" class="table table-hover small table-responsive table-bordered text-center w-100">
						<thead>
							<tr>
								<th width="3%"><i class="fa fa-plus btn btn-success btn-sm" data-rows="2"
										onclick="create_fullctm(this)"></i></th>
						...
						<tbody>
							<tr class="fullctm_row">
								<td>
									<i class="fa fa-times btn btn-danger btn-sm delbtn d-none"
								...
								<td class="text-left">
									<input type="text" name="fullctm_name<?= $id . $iid ?>"
										...
										oninput="fullctm_named(this)" placeholder="New Recipe..." required="required"
										<?= $dis ?>>
								</td>
								...
								<td>
									<label for="spicea_fullctm<?= $id ?>row1" class="spicelabel">
										<input type="radio" name="spice_fullctm<?= $id . $iid ?>"
											id="spicea_fullctm<?= $id ?>row1" class="spice addiid" value="1" <?= $dis ?>>
									</label>
								</td>
								<td>
									<label for="spiceb_fullctm<?= $id ?>row1" class="spicelabel">
										<input type="radio" name="spice_fullctm<?= $id . $iid ?>"
											id="spiceb_fullctm<?= $id ?>row1" class="spice addiid" value="2" <?= $dis ?>>
									</label>
								</td>
								<td>
									<label for="spicec_fullctm<?= $id ?>row1" class="spicelabel">
										<input type="radio" name="spice_fullctm<?= $id . $iid ?>"
											id="spicec_fullctm<?= $id ?>row1" class="spice addiid" value="3" <?= $dis ?>>
									</label>
								</td>
								...
								<td>
									<input type="number" name="persons_fullctm<?= $id . $iid ?>"
										class="person d-none addiid v0 form-control form-control-sm" min="0" value=""
										placeholder="PP" oninput="fullctm_tray_cal(this)" <?= $dis ?>>
								</td>
								<!-- TRAY ITEMS -->
								<td class="fullctm_tray">
									<input type="number" name="tray_lg_fullctm<?= $id . $iid ?>"
										class="lg_qty addiid v0 form-control form-control-sm" min="0" value="0"
										placeholder="L" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								<td class="fullctm_tray">
									<input type="number" name="tray_md_fullctm<?= $id . $iid ?>"
										class="md_qty addiid v0 form-control form-control-sm" min="0" value="0"
										placeholder="M" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								<td class="fullctm_tray">
									<input type="number" name="tray_sm_fullctm<?= $id . $iid ?>"
										class="sm_qty addiid v0 form-control form-control-sm" min="0" value="0"
										placeholder="S" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>

								<td class="fullctm_tray">
									<input type="number" name="lg_price_fullctm<?= $id . $iid ?>"
										class="lg_price addiid vnull form-control form-control-sm" step="any"
										placeholder="L" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								<td class="fullctm_tray">
									<input type="number" name="md_price_fullctm<?= $id . $iid ?>"
										class="md_price addiid vnull form-control form-control-sm" step="any"
										placeholder="M" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								<td class="fullctm_tray">
									<input type="number" name="sm_price_fullctm<?= $id . $iid ?>"
										class="sm_price addiid vnull form-control form-control-sm" step="any"
										placeholder="S" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								...
								<td colspan="2" class="fullctm_qty d-none">
									<input type="number" name="fullctm_qty<?= $id . $iid ?>"
										class="ps_qty addiid v0 form-control form-control-sm" min="0" value="0"
										placeholder="qty" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								<!-- Per serving price -->
								<td class="fullctm_qty d-none" colspan="3">
									<input type="number" name="ps_price_fullctm<?= $id . $iid ?>"
										...
										placeholder="price per item" step="any" oninput="fullctm_total(this)" <?= $dis ?>>
								</td>
								...
								<td>
									<input type="number" name="total_price_fullctm<?= $id . $iid ?>"
										...
										step="any" value="0.00" data-active="0" <?= $dis ?>>
								</td>

								<td><input type="text" name="comment_fullctm<?= $id . $iid ?>"
										class="form-control addiid emp form-control-sm" oninput="fullctm_total(this)"
										<?= $dis ?>></td>
							</tr>
						</tbody>
					</table>
			...
			<div class="col-md-6 col-sm-12 col-lg-3">
				<div class="form-group">
					<div class="input-group input-group-sm">
						...
						<input type="number" name="extra" id="extra" class="form-control" step="any" value="0.00"
							oninput="amount_cal()">
					</div>
				</div>
			</div>
			...
			<div class="col-md-6 col-sm-12 col-lg-3">
				<div class="form-group">
					<div class="input-group input-group-sm">
						...
						<input type="number" name="advance" id="advance" class="form-control" step="any" value="0.00"
							oninput="amount_cal()">
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-md-11">
				<button type="submit" id="submitbtn" class="btn btn-success float-right m-4"><i
						class="fa fa-paper-plane"></i> Place Order</button>
